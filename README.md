...
- **温度远低于固件临界点**（阈值取自板卡固件热管理配置，非泛化估计）：NPU 32.4 → 峰值 38.8 → 末值 37.0 °C，全部热区 ≤39.8 °C；`npu-thermal` 临界点 **115 °C（critical）**，`soc-thermal` 75/85 °C（passive），本次负载远低于两者。
